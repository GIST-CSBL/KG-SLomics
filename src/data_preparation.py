...
def scatter_multiomics_augmentation(cellline_Omics, KG):
    max_iters=10
    _, KGCCLE_cID = ccle_preparation(cellline_list)
    idx = torch.as_tensor(KGCCLE_cID, dtype=torch.long)
...
def generate_SL_network(SL_set, omics):
    ...
    sl_ccle_graph.x = scatter_multiomics_augmentation(omics, BaseKG)
...
def kfold_pair_split_dataset(SLs):
    ...
    # [0]: tr; [1]: val; [2]: ts; [3]: nodupEdges; [0][i]: ith fold; [0][i][j]: jth cell line
...
def neg_pair_sampling(cellline_id, args):
    ...
    if os.path.exists(MAINPATH + 'CombinedSLs/' + cellline_list[cellline_id] + '_nonSL.csv'):
        ...
        unique_pairs = remove_duplicate_pairs(cellline_GraphList[cellline_id].edge_index)
        bidirectional_uniques = torch.cat([unique_pairs.edge_index, unique_pairs.edge_index[[1,0]]], axis=1)
        sampled_unknown_edges = negative_pool_generation(bidirectional_uniques)
        sampled_full_neg_edges_pool = remove_duplicate_pairs(torch.cat([neg_edge_index_init, sampled_unknown_edges], dim=1))
        
        sampling_idx = torch.randperm(sampled_full_neg_edges_pool.size(1))
        n_supple = cellline_GraphList[cellline_id].edge_index.size(1)*args.negratio
        sampled_neg_edges_idx = sampling_idx[:n_supple]
        
        final_neg_edge_index = sampled_full_neg_edges_pool[:, sampled_neg_edges_idx]
...
def conditional_neg_sampling(true_neg, split_data, args):
...
def generate_complete_SLpDataset(tr, val, ts, args):
    ...
    for j in range(len(cellline_GraphList)):
        sampled_neg_edges_pool = neg_pair_sampling(j, args) # negative sampling for positive samples in jth cell line
...
def generate_complete_SLgDataset(tr, val, ts, args):
    print('Data processing...')
    tr_edge_info, val_edge_info, ts_edge_info = [], [], []
    opt_rotate = KGdata(MAINPATH)
    cellline_GraphList = combine_SL_labels(cellline_list)
    ...
    for j in range(len(cellline_GraphList)):
        if os.path.exists(MAINPATH + 'CombinedSLs/' + cellline_list[j] + '_nonSL.csv'):
            ...
            tr_neg_edges = conditional_neg_sampling(filtered_tr_neg, tr[j], args)
            ...
            val_neg_edges = conditional_neg_sampling(filtered_val_neg, val[j], args)
            ...
            ts_neg_edges = conditional_neg_sampling(filtered_ts_neg, ts[j], args)
