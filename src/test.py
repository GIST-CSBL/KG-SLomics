...
@torch.no_grad()
def inference(cellline, threshold, args):
    ...
    model_opt.load_state_dict(torch.load('checkpoint_latest.pt'))
    model_opt.eval()
    ...
    for data in tqdm(test_loader):
        ...
        y_pred.append(ts_out_sig>0.5)
        y_pred_prob.append(ts_out_sig)
        
        del(data)
    
    all_pairs = torch.cat(pairs, dim=1)
    results_df = pd.DataFrame(torch.transpose(torch.vstack((all_pairs, y_pred, y_pred_prob)), 0, 1).numpy(), 
    ...
    results_df_name.to_csv(MAINPATH + '/SL_prediction/'+ test_cellline +'_SL_list.csv', index=False)
    
    return results_df_name
...
if __name__=='__main__':
    ...
    inference(test_cellline, 0.5, args)
